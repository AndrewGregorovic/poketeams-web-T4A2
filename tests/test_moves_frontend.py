...
class TestTeamsFrontend(CustomBaseTestClass):
    ...
    def test_view_pokemon_move(self):
        ...
        with self.client as c:
            ...
            team, team_pokemon, move = self.get_move_slot_private()
            self.login({"email": f"test{team.owner_id}@test.com", "password": "123456"})

            with captured_templates(self.app) as templates:
                response = c.get(url_for("moves.view_pokemon_move", team_id=team.id, team_index=team_pokemon.team_index, pokemon_move_index=move.pokemon_move_index))
            ...
            team, team_pokemon, move_index = self.get_empty_move_slot()
            self.login({"email": f"test{team.owner_id}@test.com", "password": "123456"})

            response = c.get(url_for("moves.view_pokemon_move", team_id=team.id, team_index=team_pokemon.team_index, pokemon_move_index=move_index))

            self.assertEqual(response.status_code, 302)

            with captured_templates(self.app) as templates:
                response = c.get(url_for("moves.view_pokemon_move", team_id=team.id, team_index=team_pokemon.team_index, pokemon_move_index=move_index), follow_redirects=True)
                template, context = templates[0]
            ...
            team, team_pokemon, move = self.get_move_slot_public()
    ...
    def test_get_pokemon_move_list(self):
        ...
        with self.client as c:
            ...
            with captured_templates(self.app) as templates:
                response = c.get(url_for("moves.get_pokemon_move_list", team_id=team.id, team_index=team_pokemon.team_index, pokemon_move_index=random.randint(1, 4)))
                template, context = templates[0]
    ...
    def test_view_selected_pokemon_move(self):
        ...
        with self.client as c:
            ...
            with captured_templates(self.app) as templates:
                response = c.get(url_for("moves.view_selected_pokemon_move", team_id=team.id, team_index=team_pokemon.team_index, pokemon_move_index=random.randint(1, 4), move_id=random.randint(1, 826)))
                template, context = templates[0]
    ...
    def test_edit_pokemon_move_slot(self):
        ...
        with self.client as c:
            ...
            team_pokemon = self.get_random_team_pokemon(team)
            self.login({"email": f"test{team.owner_id}@test.com", "password": "123456"})
            response = c.post(url_for("moves.edit_pokemon_move_slot", team_id=team.id, team_index=team_pokemon.team_index, pokemon_move_index=random.randint(1, 4), move_id=random.randint(1, 826)))

            self.assertEqual(response.status_code, 302)

            # Test when redirect is followed
            with captured_templates(self.app) as templates:
                response = c.post(url_for("moves.edit_pokemon_move_slot", team_id=team.id, team_index=team_pokemon.team_index, pokemon_move_index=random.randint(1, 4), move_id=random.randint(1, 826)), follow_redirects=True)
                template, context = templates[0]
    ...
    def test_delete_team_slot_pokemon(self):
        ...
        with self.client as c:
            team, team_pokemon, move = self.get_move_slot_public()
            self.login({"email": f"test{team.owner_id}@test.com", "password": "123456"})
            response = c.post(url_for("moves.delete_pokemon_move_slot", team_id=team.id, team_index=team_pokemon.team_index, pokemon_move_index=move.pokemon_move_index))
            ...
            self.login({"email": f"test{team.owner_id}@test.com", "password": "123456"})
            with captured_templates(self.app) as templates:
                response = c.post(url_for("moves.delete_pokemon_move_slot", team_id=team.id, team_index=team_pokemon.team_index, pokemon_move_index=move.pokemon_move_index), follow_redirects=True)
                template, context = templates[0]
