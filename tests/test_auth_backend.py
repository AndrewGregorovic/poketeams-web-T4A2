...
class TestAuthBackend(CustomBaseTestClass):
    """
    Test cases to test the backend side of the auth controller endpoints.
    """

    def test_signup(self):
        """
        Tests that the signup endpoint correctly registers new users and reports form errors.
        """
    ...
    def test_login(self):
        ...
        response1 = self.login(user_data1)
        self.logout()
        response2 = self.login(user_data2)
        self.logout()
        response3 = self.login(user_data3)
        self.logout()

        self.assertEqual(response1.status_code, 200)
        self.assertIn(b"Poketeams Dashboard", response1.data)
